...
const Product = (props: Props) => {
    ...
    return (
        <section className="mx-auto max-w-7xl px-2 sm:px-6 pt-10 pb-10">
            <div>
                <h2 className="font-bold tracking-tight text-[#00ccff] text-center text-4xl pb-10">
                    Sản phẩm mới
                </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {showMore ? data?.data.productNew : initialProducts?.map((product) => (
                    <div key={product.id} className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow ">
                        <div>
                            <img
                                className="p-8 rounded-t-lg w-full h-[300px]"
                       ...
                       <div className="px-5 pb-5">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <Link to="#" className="block w-full rounded bg-[#00ccff] px-12 py-3 text-sm font-medium text-white shadow hover:bg-white hover:text-[#00ccff] focus:outline-none focus:ring active:bg-[#00ccff] active:text-white sm:w-auto">Chi tiết</Link>
                            </div>
                        </div>
                    </div>
                ))}
